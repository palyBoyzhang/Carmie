...
@interface CAREXQPersonaController ()

@property (nonatomic, strong) UIScrollView *crxScrollView;
@property (nonatomic, strong) UIView *crxContentView;
@property (nonatomic, strong) UIButton *crxVideoButton;
@property (nonatomic, strong) UIButton *crxFriendButton;
@property (nonatomic, assign) BOOL crxFriendAdded;

@end

@implementation CAREXQPersonaController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor colorWithRed:0x08/255.0 green:0x08/255.0 blue:0x23/255.0 alpha:1.0];
    [self crx_restoreFriendState];
    [self crx_buildViews];
}
...
- (void)crx_buildViews {
    ...
    self.crxVideoButton = [self crx_roundActionButtonWithSystemName:@"video.fill"];
    [self.crxVideoButton addTarget:self action:@selector(crx_videoTapped) forControlEvents:UIControlEventTouchUpInside];
    [crxActionStack addArrangedSubview:self.crxVideoButton];
    [self.crxVideoButton.widthAnchor constraintEqualToConstant:48].active = YES;

    self.crxFriendButton = [self crx_roundActionButtonWithSystemName:@"person.badge.plus.fill"];
    [self.crxFriendButton addTarget:self action:@selector(crx_friendTapped) forControlEvents:UIControlEventTouchUpInside];
    [crxActionStack addArrangedSubview:self.crxFriendButton];
    [self.crxFriendButton.widthAnchor constraintEqualToConstant:48].active = YES;
    ...
    [self crx_refreshFriendButtonState];
}
...
- (NSDictionary *)crx_profilePayload {
    ...
    NSArray<UIImage *> *crxHeroImages = @[
        [CAREXQImage imageNamed:@"crx_dy_2"] ?: crxAvatar,
        [CAREXQImage imageNamed:@"crx_dy_4"] ?: crxAvatar,
        [CAREXQImage imageNamed:@"crx_dy_6"] ?: crxAvatar,
        [CAREXQImage imageNamed:@"crx_dy_8"] ?: crxAvatar,
        [CAREXQImage imageNamed:@"crx_dy_10"] ?: crxAvatar,
        [CAREXQImage imageNamed:@"crx_dy_12"] ?: crxAvatar,
        [CAREXQImage imageNamed:@"crx_dy_14"] ?: crxAvatar,
        [CAREXQImage imageNamed:@"crx_dy_16"] ?: crxAvatar
    ];
    ...
    for (NSInteger crxStep = 0; crxStep < 4; crxStep++) {
        ...
        UIImage *crxImage = [CAREXQImage imageNamed:[NSString stringWithFormat:@"crx_dy_%ld", (long)crxImageIndex]] ?: crxAvatar;
...
- (NSString *)crx_friendStateKey {
    NSString *crxName = self.crxUserItem[@"crxName"] ?: @"default";
    return [NSString stringWithFormat:@"%@%@", CAREXQPersonaFriendStatePrefix, crxName];
}

- (void)crx_restoreFriendState {
    self.crxFriendAdded = [NSUserDefaults.standardUserDefaults boolForKey:[self crx_friendStateKey]];
}

- (void)crx_refreshFriendButtonState {
    if (!self.crxFriendButton) {
        return;
    }
    NSString *crxImageName = self.crxFriendAdded ? @"person.fill.checkmark" : @"person.badge.plus.fill";
    [self.crxFriendButton setImage:[UIImage systemImageNamed:crxImageName] forState:UIControlStateNormal];
    self.crxFriendButton.tintColor = self.crxFriendAdded ? UIColor.whiteColor : [UIColor colorWithRed:1.0 green:0.35 blue:0.72 alpha:1.0];
    self.crxFriendButton.backgroundColor = self.crxFriendAdded ? [UIColor colorWithRed:0.53 green:0.20 blue:0.95 alpha:1.0] : [UIColor colorWithRed:0x21/255.0 green:0x10/255.0 blue:0x38/255.0 alpha:1.0];
    self.crxFriendButton.layer.borderColor = (self.crxFriendAdded ? [UIColor colorWithWhite:1 alpha:0.22] : [UIColor colorWithRed:0.89 green:0.22 blue:0.88 alpha:0.55]).CGColor;
}

- (BOOL)crx_requireLogin {
...
- (void)crx_friendTapped {
    ...
    self.crxFriendAdded = !self.crxFriendAdded;
    [NSUserDefaults.standardUserDefaults setBool:self.crxFriendAdded forKey:[self crx_friendStateKey]];
