...
@interface CAREXQBlueprintParticipantCard : UIControl

- (void)crx_configureWithItem:(NSDictionary *)crxItem;

@end

@interface CAREXQBlueprintParticipantCard ()
...
@property (nonatomic, strong) NSLayoutConstraint *crxCoverHeightConstraint;

@end

@implementation CAREXQBlueprintParticipantCard
...
- (void)crx_setupViews {
    ...
    self.crxNameLabel.font = [UIFont systemFontOfSize:13 weight:UIFontWeightSemibold];
    self.crxNameLabel.textColor = UIColor.whiteColor;
    self.crxNameLabel.numberOfLines = 1;
    self.crxNameLabel.lineBreakMode = NSLineBreakByTruncatingTail;
    ...
    self.crxCoverHeightConstraint = [self.crxCoverView.heightAnchor constraintEqualToConstant:114];
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [crxBottomShade.heightAnchor constraintEqualToConstant:50],

        [self.crxAvatarView.leadingAnchor constraintEqualToAnchor:self.leadingAnchor constant:8],
        [self.crxAvatarView.bottomAnchor constraintEqualToAnchor:self.bottomAnchor constant:-15],
        [self.crxAvatarView.widthAnchor constraintEqualToConstant:18],
        [self.crxAvatarView.heightAnchor constraintEqualToConstant:18],
        ...
        [self.crxNameLabel.trailingAnchor constraintLessThanOrEqualToAnchor:self.crxHeartButton.leadingAnchor constant:-6],

        [self.crxMetaLabel.leadingAnchor constraintEqualToAnchor:self.crxNameLabel.leadingAnchor],
        [self.crxMetaLabel.topAnchor constraintEqualToAnchor:self.crxNameLabel.bottomAnchor constant:1],
        [self.crxMetaLabel.bottomAnchor constraintLessThanOrEqualToAnchor:self.bottomAnchor constant:-4],

        [self.crxHeartButton.trailingAnchor constraintEqualToAnchor:self.trailingAnchor constant:-8],
        [self.crxHeartButton.centerYAnchor constraintEqualToAnchor:self.crxNameLabel.centerYAnchor],
...
- (void)crx_configureWithItem:(NSDictionary *)crxItem {
    self.crxCoverView.image = crxItem[@"crxCoverImage"] ?: [CAREXQImage imageNamed:@"crx_stage_aig"];
    self.crxAvatarView.image = [CAREXQImage imageNamed:crxItem[@"crxAvatarName"]] ?: [CAREXQImage imageNamed:@"crx_head_1"];
    self.crxNameLabel.text = crxItem[@"crxName"] ?: @"User";
    self.crxMetaLabel.text = crxItem[@"crxLikeText"] ?: @"12.5";
    self.crxCoverHeightConstraint.constant = self.bounds.size.width > 112.0 ? 120.0 : 114.0;
}

@end

@interface CAREXQBlueprintController ()
...
@implementation CAREXQBlueprintController
...
- (void)crx_prepareData {
    if (self.crxStageItem.count == 0) {
        self.crxStageItem = @{
            ...
            @"crxCoverImage": [CAREXQImage imageNamed:@"crx_stage_aig"] ?: UIImage.new
        };
    }
    ...
    for (NSInteger idx = 0; idx < 4; idx++) {
        ...
        [crxCards addObject:@{
            @"crxCoverImage": [CAREXQImage imageNamed:[NSString stringWithFormat:@"crx_dy_%ld", (long)crxImageIndex]] ?: self.crxStageItem[@"crxCoverImage"] ?: UIImage.new,
            @"crxAvatarName": [NSString stringWithFormat:@"crx_head_%ld", (long)crxHeadIndex],
            @"crxName": crxNames[crxHeadIndex - 1],
            @"crxLikeText": @"12.5"
        }];
    }
    self.crxParticipantItems = [CAREXQController crx_filterItems:crxCards.copy nameKey:@"crxName"];
}
...
- (void)crx_setupViews {
    ...
    [self.crxSendButton setImage:[CAREXQImage imageNamed:@"crx_send_btn@3x"] forState:(UIControlStateNormal)];
    ...
    for (NSDictionary *crxCardItem in self.crxParticipantItems) {
        CAREXQBlueprintParticipantCard *crxCard = [[CAREXQBlueprintParticipantCard alloc] initWithFrame:CGRectZero];
        ...
        [crxCardConstraints addObject:[crxCard.widthAnchor constraintEqualToConstant:120]];
        if (crxPreviousCard) {
            [crxCardConstraints addObject:[crxCard.leadingAnchor constraintEqualToAnchor:crxPreviousCard.trailingAnchor constant:12]];
        } else {
...
- (void)crx_addInterestAvatarsToView:(UIView *)crxRowView {
    for (NSInteger idx = 0; idx < 5; idx++) {
        NSInteger crxHeadIndex = (([self crx_videoIndex] + idx) % 8) + 1;
        UIImageView *crxAvatarView = [[UIImageView alloc] initWithImage:[CAREXQImage imageNamed:[NSString stringWithFormat:@"crx_head_%ld", (long)crxHeadIndex]]];
...
- (void)crx_openPlayer {
    CAREXQStagePlayerController *crxController = [[CAREXQStagePlayerController alloc] init];
...
- (void)crxSendButtonTapped {
    ...
    UIImage *crxAvatar = self.crxStageItem[@"crxAvatarImage"] ?: [CAREXQImage imageNamed:self.crxStageItem[@"crxAvatarName"]] ?: [CAREXQImage imageNamed:@"crx_head_1"] ?: UIImage.new;
