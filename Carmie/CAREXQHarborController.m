...
@interface CAREXQHarborController ()

@property (nonatomic, strong) UIButton *crxNameButton;
@property (nonatomic, assign) BOOL crxLoadingHarborData;

@end

@implementation CAREXQHarborController

- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    [self.navigationController setNavigationBarHidden:YES animated:NO];
    [self crx_reloadHarborData];
}

- (void)viewDidLoad {
    [super viewDidLoad];
    [self setCAREXQHarborViews];
    [self crx_reloadHarborData];
}

- (void)setCAREXQHarborViews {
    UIImageView *crxImageView = [[UIImageView alloc] init];
    crxImageView.image = [CAREXQImage imageNamed:@"crx_harbor_logo"];
    ...
    self.crxNameButton = [UIButton buttonWithType:(UIButtonTypeCustom)];
    self.crxNameButton.titleLabel.font = [UIFont systemFontOfSize:20 weight:(UIFontWeightBold)];
    [self.crxNameButton setTitleColor:UIColor.whiteColor forState:UIControlStateNormal];
    [self.crxNameButton addTarget:self action:@selector(crxNameButtonTapped) forControlEvents:(UIControlEventTouchUpInside)];
    [self.view addSubview:self.crxNameButton];
    self.crxNameButton.translatesAutoresizingMaskIntoConstraints = NO;
    [NSLayoutConstraint activateConstraints:@[
        [self.crxNameButton.topAnchor constraintEqualToAnchor:crxImageView.bottomAnchor constant:12],
        [self.crxNameButton.centerXAnchor constraintEqualToAnchor:self.view.centerXAnchor]
    ]];
    ...
    [NSLayoutConstraint activateConstraints:@[
        [crxView.topAnchor constraintEqualToAnchor:self.crxNameButton.bottomAnchor constant:20],
    ...
    [crxedtButton setImage:[CAREXQImage imageNamed:@"crx_harbor_edt"] forState:(UIControlStateNormal)];
    ...
    [crxharbosepButton setImage:[CAREXQImage imageNamed:@"crx_harbor_sep"] forState:(UIControlStateNormal)];
    ...
    crxpatImageView.image = [CAREXQImage imageNamed:@"crx_harbor_pat"];
    ...
    crxzhsImageView.image = [CAREXQImage imageNamed:@"crx_harbor_zhs"];
...
- (void)crxNameButtonTapped {
    ...
    CAREXQIdentityEditorController *crxVC = [[CAREXQIdentityEditorController alloc] init];
    [self.navigationController pushViewController:crxVC animated:YES];
}
...
- (void)crx_refreshLoginDisplay {
    if (!self.crxNameButton) {
        return;
    }

    NSString *crxSavedNickname = [NSUserDefaults.standardUserDefaults stringForKey:CAREXQHarborNicknameKey];
    NSString *crxNickname = [crxSavedNickname stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    NSString *crxTitle = ([self crx_isLoggedIn] && crxNickname.length > 0) ? crxNickname : @"Login now.";
    [self.crxNameButton setTitle:crxTitle forState:UIControlStateNormal];
}

- (void)crx_reloadHarborData {
    if (self.crxLoadingHarborData) {
        return;
    }
    self.crxLoadingHarborData = YES;
    [self crx_showLoadingWithText:@"Loading..."];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.12 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        self.crxLoadingHarborData = NO;
        [self crx_refreshLoginDisplay];
...
- (void)crxharbosepButtonTapped {
    ...
    CAREXQSettingController *crxVC = [[CAREXQSettingController alloc] init];
    [self.navigationController pushViewController:crxVC animated:YES];
}
...
- (void)crxzhsViewTapped {
    ...
    CAREXQAccessCenterController *crxVC = [[CAREXQAccessCenterController alloc] init];
    [self.navigationController pushViewController:crxVC animated:YES];
}
